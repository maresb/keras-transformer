...
class TransformerTransition(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        step1 = self.activation(
            K.bias_add(
                K.dot(K.reshape(inputs, tf.stack((-1, d_model))),
                      self.weights1),
                self.biases1,
        ...
        result = K.reshape(
            step2,
            tf.stack((-1, input_shape[-2], input_shape[-1]))
        )
        return result
...
class TransformerBlock(Layer):
    ...
    def __call__(self, _input, mask=None):
        output = self.attention_layer(_input, mask=mask)
        post_residual1 = (
